...
create_user_response = httpx.post(f"{settings.http_client.url}api/v1/users", json=create_user_payload)
...
create_file_response = httpx.post(
    f"{settings.http_client.url}api/v1/files",
    data={"filename": "image.png", "directory": "courses"},
    files={"upload_file": open(settings.test_data.image_png_file.read_bytes())},
